...
namespace Logging
{
    ...
    const std::unordered_map<Level, std::string, level_hashing_function> prefix{
        {Level::ERROR, " [ERROR] "},
        {Level::WARN, " [WARN] "},
        {Level::INFO, " [INFO] "},
        {Level::DEBUG, " [DEBUG] "},
    ...
    */
    ...
    {
        std::chrono::system_clock::time_point tp = std::chrono::system_clock::now(); // Implementation dependant: microseconds or nanoseconds.
    ...
    public:
        ...
        virtual void log(const std::string &message, const Level level, const std::string &name);
        virtual void log(const std::string &message, const Level level) override;
